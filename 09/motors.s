...
min:
        ...
        bge continua            @ Se r0 menor que o limiar: Salta para end

vira:                           @ Analisa para que lado o robo deve virar
        mov r0, #0              @ Define em r0 o identificador do sonar a ser consultado.
        mov r7, #125            @ Identifica a syscall 125 (read_sonar).
        svc 0x0 

        cmp r0, r6              @ Verifica se ha algo do lado esquerdo do robo
        bge vira_esquerda       @ Se não tiver, vira para a esquerda
                                @ Se tiver, vira ppara a direita

vira_direita:                   @ Trava o motor0 e movimenta apenas o motor1
        mov r0, #0            
        mov r1, #10
        mov r7, #124        
        svc 0x0
...
vira_esquerda:                  @ Trava o motor1 e movimenta apenas o motor0
        mov r0, #10            
        mov r1, #0
        mov r7, #124        
        svc 0x0
...
continua:                       @ Senao define uma velocidade para os 2 motores
        mov r0, #36            
        mov r1, #36
        mov r7, #124        
        svc 0x0
